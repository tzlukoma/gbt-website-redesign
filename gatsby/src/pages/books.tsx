...
function BookPage({ data }) {
  const books = data.books.edges.sort((a, b) =>
      ...
      {books.map((book) => {
        ...
        return (
          <div className="my-10 p-10 bg-gray-50 border-gray-100 border-solid border-2 sm:w-1/3 m-auto">
            ...
            <h2 className="mt-0 text-xl text-center">{`by ${authors.map(
              (author) => author.name,
            )}`}</h2>
            <div className="max-w-sm mb-10 m-auto ">
              ...
              <div className="text-center mt-10">
                <a
                  href={url}
                  className="flex-shrink-0 bg-secondary-200 hover:bg-primary-600 border-secondary-200 rounded hover:border-primary-600 lg:text-lg border-4 text-primary-500 hover:text-white py-1 px-5 lg:px-10"
                >
                  {`Buy ${shortTitle}`}
                </a>
              </div>
