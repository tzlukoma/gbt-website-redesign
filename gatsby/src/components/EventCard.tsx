  ...
  return (
    <div className="p-8  bg-light-100 border-dashed border-4 border-grey-100 shadow-md">
      ...
      <div className="pb-2 text-4xl font-bold font-display">
        <span>{displayLocalTimeZone(event?.node?.date, 'D MMM')}</span>
      ...
      <div className="mb-5">
        <p className="text-lg text-primary-600">
          {event?.node?.synopsis && event?.node?.synopsis[0].children[0].text}
        </p>
      </div>
      {event?.node?.registerUrl && (
        <button className="mb-5 px-5 py-3 rounded bg-primary-700 hover:bg-primary-100 text-white">
          <a href={event?.node?.registerUrl}>Register Here</a>
        </button>
      )}
      ...
      <div className="col-start-1 col-span-2">
        ...
        >
          {event?.node?.publishPlatform?.platformImage ? (
            <GatsbyImage
              image={
                event?.node?.publishPlatform?.platformImage?.asset
                  ?.gatsbyImageData
              }
              alt="platform icon"
